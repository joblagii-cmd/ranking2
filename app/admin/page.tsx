...
function generateBatch(
  ...
  const currency = CURRENCY[countryCode]||"USD";
  const cities = CITIES[countryCode]||["City"];
  const salaryRanges = SALARY[countryCode]||SALARY["US"];
  const jobs = [];
  ...
  for (let i = start; i < Math.min(start + batchSize, total); i++) {
    ...
    const company = rand(companiesList);
    ...
    const schema = {
      ...
      ...(isRemote&&{applicantLocationRequirements:{"@type":"Country",name:countryName},jobLocationType:"TELECOMMUTE"}),
      baseSalary:{"@type":"MonetaryAmount",currency,value:{"@type":"QuantitativeValue",minValue:sMin,maxValue:sMax,unitText:"YEAR"}},
    };

    jobs.push({id,slug,title,company,countryCode,country:countryName,city,isRemote,employmentType:empType,salaryMin:sMin,salaryMax:sMax,currency,description:desc,skills:skills.split(", "),publishedAt:publishedAt.toISOString(),validThrough:validThrough.toISOString(),schema});
  }
  return jobs;
}
...
export default function AdminPage() {
  ...
  const [totalToPublish] = useState(5000);
  ...
  const publishJobs = async () => {
    ...
    try {
      ...
      for (let b = 0; b < totalBatches; b++) {
        ...
        const batch = generateBatch(b, BATCH, TOTAL, selectedCountry, country.name, countryCompanies, publishedAt, validThrough);
        allJobs.push(...batch);
        setPublishedCount(allJobs.length);

        // Small delay every batch so UI can update
      ...
      if (!cancelRef.cancelled) {
        ...
        const newLog: PublishLog = {
          ...
          remoteJobs: allJobs.filter((j:any)=>j.isRemote).length,
          normalJobs: allJobs.filter((j:any)=>!j.isRemote).length,
        };
        ...
        a.href = url;
        a.download = `jobs-${selectedCountry.toLowerCase()}-${Date.now()}.json`;
        a.click();
        URL.revokeObjectURL(url);

        setPublishResult({
          success: true,
          message: `✅ ${allJobs.length.toLocaleString()} jobs generated for ${country.name}! JSON file downloaded.`,
          stats: {
            total: allJobs.length,
            remote: allJobs.filter((j:any)=>j.isRemote).length,
            normal: allJobs.filter((j:any)=>!j.isRemote).length,
          }
        });
      }
    } catch(e) {
      console.error(e);
      setPublishResult({success:false, message:"Something went wrong. Please try again."});
    } finally {
      setPublishing(false);
    }
  };

  const cancelPublish = () => { cancelRef.cancelled = true; };

  const progress = Math.round((publishedCount/totalToPublish)*100);
              ...
              <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:16,padding:24}}>
                ...
                {publishing&&(
                  <div style={{marginBottom:16}}>
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:8,fontSize:13,color:"#9ca3af"}}>
                      <span>⚡ Generating jobs in batches of 20...</span>
                      <span style={{fontFamily:"'Space Grotesk', sans-serif",fontWeight:600,color:"#a5b4fc"}}>{publishedCount.toLocaleString()} / {totalToPublish.toLocaleString()}</span>
                    </div>
                    <div style={{height:8,background:"rgba(255,255,255,0.06)",borderRadius:99,overflow:"hidden"}}>
                      <div style={{height:"100%",borderRadius:99,background:"linear-gradient(90deg, #6366f1, #10b981)",width:`${progress}%`,transition:"width 0.1s ease"}}/>
                    </div>
                    <div style={{display:"flex",justifyContent:"space-between",marginTop:6,fontSize:12,color:"#4b5563"}}>
                      <span>{progress}% complete</span>
                      <button onClick={cancelPublish} style={{background:"none",border:"none",color:"#ef4444",cursor:"pointer",fontSize:12,padding:0}}>✕ Cancel</button>
                    </div>
                  </div>
                )}
                ...
                {publishResult&&(
                  <div style={{marginBottom:16,padding:"14px 16px",borderRadius:10,background:publishResult.success?"rgba(16,185,129,0.08)":"rgba(239,68,68,0.08)",border:`1px solid ${publishResult.success?"rgba(16,185,129,0.25)":"rgba(239,68,68,0.25)"}`,color:publishResult.success?"#34d399":"#f87171",fontSize:14}}>
                    ...
                    {publishResult.stats&&<div style={{marginTop:6,fontSize:12,color:"#9ca3af"}}>{publishResult.stats.remote.toLocaleString()} remote · {publishResult.stats.normal.toLocaleString()} on-site · with JobPosting JSON-LD schema</div>}
                  </div>
                )}
                ...
                <button onClick={publishJobs} disabled={publishing} style={{width:"100%",padding:"15px",borderRadius:12,border:"none",background:publishing?"rgba(255,255,255,0.05)":"linear-gradient(135deg, #6366f1, #4f46e5)",color:publishing?"#4b5563":"white",fontSize:16,fontWeight:700,cursor:publishing?"not-allowed":"pointer",fontFamily:"'Space Grotesk', sans-serif",boxShadow:publishing?"none":"0 4px 24px rgba(99,102,241,0.3)"}}>
                  {publishing?`⏳ Publishing... ${publishedCount.toLocaleString()} done`:"🚀 Publish 5,000 Jobs"}
                </button>
                <p style={{textAlign:"center",fontSize:12,color:"#4b5563",marginTop:10,marginBottom:0}}>
                  Generates in batches of 20 · JSON file auto-downloads · JobPosting schema included
                </p>
              </div>
            </div>
          ) : (
