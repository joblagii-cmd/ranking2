...
interface Job {
  ...
  countryCode: string; country: string; city: string; isRemote: boolean;
  employmentType: string; seniority: string; salaryMin: number; salaryMax: number;
  currency: string; description: string; skills: string[];
  publishedAt: string; validThrough: string;
  schema: object; chunkFile?: string;
}
...
async function getJob(slug: string): Promise<Job | null> {
  ...
  // Find which country/chunk this slug belongs to by checking all index files
  const countries = ["US","CA","GB","IN","AU","DE","FR","AE","SG","NZ","AT","BE","CH","NL","SE","NO","DK","FI","IE","IT","ES","PT","PL","CZ","HU","RO","GR","HR","SK","BG","RS","UA","LU","IS","MT","CY","EE","LV","LT","SI","MK","AL","BA","ME","JP","CN","KR","MY","TH","ID","PH","VN","PK","BD","LK","MM","KH","HK","TW","MN","NP","UZ","KZ","GE","AM","AZ","IL","JO","LB","KW","QA","BH","OM","SA","IQ","IR","AF","BR","MX","AR","CL","CO","PE","VE","EC","BO","PY","UY","CR","PA","GT","HN","SV","NI","CU","DO","JM","TT","NG","ZA","EG","KE","GH","TZ","ET","MA","DZ","TN","CM","CI","SN","UG","ZM","ZW","SD","AO","MZ","MG","RW","FJ","PG"];
  for (const cc of countries) {
    try {
      const idxRes = await fetch(`${RAW}/${owner}/${repo}/main/data/index-${cc}.json`, { next: { revalidate: 300 } });
      if (!idxRes.ok) continue;
      const index: Job[] = await idxRes.json();
      const meta = index.find(j => j.slug === slug);
      if (!meta) continue;

      // Found the job in this index — now get full data from chunk file
      if (meta.chunkFile) {
        const chunkRes = await fetch(`${RAW}/${owner}/${repo}/main/data/${meta.chunkFile}`, { next: { revalidate: 3600 } });
        if (chunkRes.ok) {
          const chunk: Job[] = await chunkRes.json();
          const fullJob = chunk.find(j => j.slug === slug);
          if (fullJob) return fullJob;
        }
      }
      // Fallback: return meta (may not have description)
      return meta;
    } catch { continue; }
  }
  return null;
...
export default async function JobPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(job.schema) }} />
        ...
        <main style={{ maxWidth: 860, margin: "0 auto", padding: "48px 24px", position: "relative", zIndex: 1 }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-start", gap: 20, marginBottom: 24 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                  ...
                  <span style={{ padding: "4px 12px", borderRadius: 99, fontSize: 12, background: "rgba(255,255,255,0.06)", color: "#9ca3af", border: "1px solid rgba(255,255,255,0.08)" }}>{empLabel[job.employmentType] || job.employmentType}</span>
                  <span style={{ padding: "4px 12px", borderRadius: 99, fontSize: 12, background: "rgba(255,255,255,0.06)", color: "#9ca3af", border: "1px solid rgba(255,255,255,0.08)" }}>🌍 {job.country}</span>
                </div>
              </div>
            </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 280px", gap: 20 }}>
            ...
            <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, padding: "28px" }}>
              <h2 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 17, fontWeight: 700, margin: "0 0 20px" }}>Job Description</h2>
              <div style={{ fontSize: 14, lineHeight: 1.9, color: "#c4c4d4", whiteSpace: "pre-line" }}>{job.description}</div>
            </div>

            {/* Sidebar */}
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, padding: "20px" }}>
                <h3 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 13, fontWeight: 700, margin: "0 0 16px", color: "#9ca3af", textTransform: "uppercase", letterSpacing: "0.05em" }}>Job Details</h3>
                {[
                  ...
                  <div key={label} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid rgba(255,255,255,0.05)", fontSize: 13 }}>
                    ...
                    <span style={{ color: "#e8e8f0", fontWeight: 500, textAlign: "right", maxWidth: 150 }}>{value}</span>
                  </div>
                ))}
              </div>

              <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, padding: "20px" }}>
                <h3 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 13, fontWeight: 700, margin: "0 0 14px", color: "#9ca3af", textTransform: "uppercase", letterSpacing: "0.05em" }}>Skills Required</h3>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              ...
              <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, padding: "20px" }}>
                <h3 style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 13, fontWeight: 700, margin: "0 0 14px", color: "#9ca3af", textTransform: "uppercase", letterSpacing: "0.05em" }}>About Company</h3>
